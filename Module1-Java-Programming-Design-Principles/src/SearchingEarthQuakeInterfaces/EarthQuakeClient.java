...
public class EarthQuakeClient {
	...
	public ArrayList<QuakeEntry> filteredList(ArrayList<QuakeEntry> quakeData, Filter f){
		
		ArrayList<QuakeEntry> newFilterList = new ArrayList<QuakeEntry>();
		for(QuakeEntry entry : quakeData)
		{
			if(f.satisfies(entry))
				newFilterList.add(entry);
		}
		
		return newFilterList;
	}
	
	//Similar methods to match with an interface as shown above into 1 methods that takes an interface Filter f, that calls whatever method in which 
	//the interface was made from such as " FIlter f = new getMinMag(4.0);" and then calling "ArrayList<QuakeEntry newList = fitleredList(quakeData, f);"
	// if want the distance then create a new interfeace object " f = new getDistanceTo(myLocation,MaxDistance);"
...
//    }
	...
	//Calling a Single Method that incorprates an interface
	public void findFilterData() {
		...
		Filter f2 = new minDistanceTo(city,1000*1000);
		ArrayList<QuakeEntry> listDistanceTo = filteredList(list,f2);
	}
